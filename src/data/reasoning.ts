...
export function buildReasoningTrace(
  ...
  return [
    ...
    { text: `  Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...4pEBv0`, type: 'result', delay: 120 },
    { text: `  ✓ Session verified: token claims matched`, type: 'result', delay: 150 },
    ...
    { text: `> get_hex_physics("${cell.id}")`, type: 'command', delay: 350 },
    { text: `  slope=${cell.slope.toFixed(3)} fuel=${cell.fuelProxy.toFixed(3)} wind=${cell.wind.toFixed(2)}m/s`, type: 'result', delay: 300 },
    { text: `  thermal_inertia=${cell.thermalInertia.toFixed(3)} housing_units=${cell.housingUnits}`, type: 'result', delay: 250 },
    { text: `> compute_ips()`, type: 'command', delay: 300 },
    { text: `  IPS = ${cell.ips.toFixed(3)}  [Rothermel-inspired fusion]`, type: 'result', delay: 350 },
    { text: `> get_nearest_stations(lat, lon)`, type: 'command', delay: 300 },
    { text: `  Found ${cell.staffedStations} staffed USFA station(s) within 15mi radius`, type: 'result', delay: 350 },
    { text: `> osrm_route(drive_time)`, type: 'command', delay: 300 },
    { text: `  nearest drive-time = ${cell.driveTimeMin.toFixed(1)} min (NFPA-1 req: 6 min)`, type: cell.driveTimeMin > 6 ? 'warn' : 'result', delay: 350 },
    { text: `> compute_rcs()`, type: 'command', delay: 300 },
    { text: `  RCS = ${cell.rcs.toFixed(3)}  [response capacity vs. NFPA-1]`, type: 'result', delay: 350 },
    { text: `> compute_gap_score()`, type: 'command', delay: 350 },
    { text: `  CCG = IPS × (1 − RCS) = ${cell.ips.toFixed(2)} × ${(1 - cell.rcs).toFixed(2)}`, type: 'info', delay: 400 },
    { text: `  CCG = ${cell.ccg.toFixed(3)}  ${cell.ccg >= 0.75 ? '⚠ SEVERE GAP' : cell.ccg >= 0.5 ? '⚠ HIGH GAP' : '✓ acceptable'}`, type: cell.ccg >= 0.5 ? 'warn' : 'result', delay: 450 },
  ];
}
